...
async function updateDataBaseSavedDataNames() {
    ...
    const totalBatches = 10; // Reduced total batches
    ...
    for (let i = 0; i < totalBatches; i++) {
        ...
        try {
            ...
            // Use prepend for the first batch (newest data goes to top), append for the rest (maintains newest-first order)
            filterAndStorePackageNames(sheetData, i === 0 ? 'prepend' : 'append');
    ...
    data.forEach(row => {
        ...
        if (!document.getElementById(packageName)) {
            ...
            if (prependStatus === 'prepend') {
                allGoogleSheetStoredDataNamesForImportingDataDiv.prepend(h3Element);
            } else {
